...
\begin{itemize}
    ...
    \item \textbf{CLIP ViT-L/14 and ViT-G/14} \cite{radford2021clip}: Strong baseline dual-encoders widely used in multimodal retrieval. We evaluate their zero-shot performance and further apply LoRA fine-tuning to assess whether lightweight adaptation improves VWSD.
    ...
    \item \textbf{Text-Augmented Retrieval}: Expands the query with LLM-generated paraphrases, explanations, and captions, and aggregates similarity scores across augmented variants.
    \item \textbf{Text-to-Image Matching}: Generates images from the query via Gemini 2.5 API and performs image–image similarity matching against candidate images.
    \item \textbf{Cascade Reranking}: A two-stage pipeline combining SigLIP2 retrieval with VLM reranking of top-$K$ candidates.
    \item \textbf{LoRA Fine-tuning}: Parameter-efficient adaptation applied to both SigLIP2 and CLIP variants, combined with VLM-generated text augmentation (captions, definitions, paraphrases).
...
We apply LoRA to both CLIP and SigLIP2 models, targeting attention projection layers (\texttt{q\_proj}, \texttt{k\_proj}, \texttt{v\_proj}, \texttt{out\_proj}) while freezing the vision encoder entirely. For CLIP, we fine-tune \texttt{laion/CLIP-ViT-L-14}; for SigLIP2, we fine-tune \texttt{google/siglip2-base-patch16-512}. Training uses SigLIP-style sigmoid contrastive loss:
\begin{equation}
\mathcal{L} = -\log \sigma(s^+) - \sum_{j=1}^{N} \log \sigma(-s_j^-)
\end{equation}
where $s^+ = f_t(q)^\top f_v(I^+)$ is the positive similarity and $s_j^-$ are similarities with $N=5$ hard negatives (other candidate images for the same word). We disable in-batch negatives to avoid false negative issues when different instances share semantically similar correct images.

\subsubsection{Training Configuration}

We use LoRA rank $r=16$, scaling factor $\alpha=32$, and dropout 0.05. Training employs AdamW with learning rate $2 \times 10^{-5}$, batch size 8, and gradient accumulation over 4 steps (effective batch size 32). We train for 5--10 epochs with bfloat16 mixed precision and 10\% linear warmup. The complete configuration adds only $\sim$2M trainable parameters to the $\sim$878M frozen base model.

\subsubsection{VLM-Based Text Augmentation}

To combat overfitting on the limited training data (12,869 instances), we augment text queries using VLMs. We generate three augmentation types offline and cache them for efficient training:

\textbf{Captions} (Qwen3-VL-8B): Natural descriptions of gold images (e.g., ``A solitary wading bird stands alert on moss-covered rocks near the water's edge''), providing image-grounded alternatives to the minimal original context. We generate one caption per instance using max 150 tokens.

\textbf{Definitions} (Qwen3-8B): Visual sense definitions that describe what would appear in an image for the intended meaning (e.g., ``Long-necked wading bird with gray plumage in wetland habitat''). These offer semantically precise but stylistically different queries.

\textbf{Paraphrases} (Qwen3-8B): Three rephrased variants of the context phrase that preserve the target word (e.g., ``midnight bluegrass'' $\rightarrow$ ``Bluegrass at midnight'', ``Midnight-colored bluegrass''), increasing lexical diversity.

In total, we generate augmentations for 12,840 instances (99.8\% coverage), yielding approximately 51,000 augmented text variants (1 caption + 1 definition + 3 paraphrases per instance). During training, original queries are replaced with augmented versions with probability $p_{\text{aug}}=0.5$, with uniform random selection among available augmentation types.
...
\textbf{Finding 7: The preservation-adaptation trade-off.} LoRA fine-tuning reveals a critical tension between task adaptation and representation preservation. Without text augmentation, SigLIP2 + LoRA achieves only 70.63\% Hit@1 after 1 epoch and shows rapid overfitting: training loss continues decreasing while validation performance degrades in subsequent epochs. With VLM-generated text augmentation (captions, definitions, paraphrases), the same model reaches 75.81\% Hit@1---a 5.18-point improvement---and exhibits slower overfitting with a broader performance plateau.

This reveals that VWSD success depends heavily on pre-trained representations: the general visual-linguistic associations learned from web-scale data are more valuable than task-specific patterns learned from 12,869 instances. Aggressive adaptation destroys these associations, causing embedding drift. Text augmentation works by increasing effective query diversity: rather than seeing ``river bank'' repeatedly, the model encounters ``Sloped land beside a flowing stream'' (definition) or ``A muddy riverbank with green vegetation'' (caption), preventing memorization of surface patterns. Notably, caption augmentation (75.81\%) slightly outperforms definition augmentation (73.65\%), suggesting that natural image descriptions better match the model's pre-training distribution than structured definitions.

This finding suggests a general principle for low-data multimodal tasks: preserve pre-trained representations through conservative adaptation (low learning rate, few epochs) and augment inputs with diverse text variants rather than aggressively adapting weights.
...
\subsubsection{Augmentation Statistics}

\begin{itemize}
    \item \textbf{Training instances:} 12,869
    \item \textbf{Augmented coverage:} 12,840 instances (99.8\%)
    \item \textbf{Total augmented variants:} $\sim$51,000 (1 caption + 1 definition + 3 paraphrases per instance)
    \item \textbf{Cache size:} 39 MB (17 MB merged index + 22 MB individual caches)
    \item \textbf{Generation time:} $\sim$4 hours on NVIDIA RTX 4090
\end{itemize}

During training, each sample has a 50\% probability of using an augmented query instead of the original context. This effectively quadruples the diversity of text queries seen during training, helping prevent overfitting to surface patterns in the limited training data.
